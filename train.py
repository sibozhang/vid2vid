    ...
    for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
        ...
        for idx, data in enumerate(dataset, start=epoch_iter):        
            ...
            for i in range(0, n_frames_total-t_len+1, n_frames_load):
                ...
                if opt.add_hand_disc:
                    loss_G += loss_dict['G_lh_GAN'] + loss_dict['G_lh_GAN_Feat']
                    loss_D += (loss_dict['D_lh_fake'] + loss_dict['D_lh_real']) * 0.5
                    loss_G += loss_dict['G_rh_GAN'] + loss_dict['G_rh_GAN_Feat']
                    loss_D += (loss_dict['D_rh_fake'] + loss_dict['D_rh_real']) * 0.5
